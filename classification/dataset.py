...
class ImageNetTrainSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgpath = os.path.join(self.opt.base, imgname)                      # path of one image
        img = Image.open(imgpath).convert('RGB')                            # read one image (RGB)
        img = np.array(img)                                                 # read one image
        if self.opt.crop_size > 0:
            rand_h = random.randint(0, img.shape[0] - self.opt.crop_size)
            rand_w = random.randint(0, img.shape[1] - self.opt.crop_size)
